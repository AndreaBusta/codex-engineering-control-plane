...
| malicious Git config or filter redirige la observación | comandos, filtros o helpers interpretan contenido hostil | entorno Git cerrado; formas read-only exactas; ejecución ligada por `fchdir` al descriptor retenido; límite pre-buffer y kill de process group; Survey intercala tres guardas de filtros con dos `status` byte-exactos | carrera same-UID posterior a la última guarda o binario Git/Python comprometido |
| topología o estado Git no materializado se usa antes de la guarda | Git hidrata metadata, sigue un symlink o un árbol hostil agota recursos | Survey y Stable Pause ejecutan primero la guarda Git-state; `.git`/`commondir` filesystem-first, controles no-follow bounded y recorrido por descriptores con cotas de entradas, profundidad y deadline; cualquier tipo inseguro es `UNKNOWN` | un syscall bloqueado dentro del kernel puede superar el deadline cooperativo |
| submodule, alternate o sustitución de worktree saca una observación fuera del inventario | `status` ejecuta filtros anidados, Git lee objetos de otro árbol o se atribuye limpieza al checkout equivocado | Survey rechaza gitlinks y alternates antes de Git, liga el Git-dir específico y su backlink, `HEAD`, branch e índice antes/después de `status`, usa OIDs congelados y revalida worktrees, refs y stash al final | Survey conserva timeouts por comando, no un snapshot atómico ni un deadline wall-clock global |
