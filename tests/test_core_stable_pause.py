...
class StablePauseCliTests(unittest.TestCase):
    def test_dataless_git_state_fails_closed_before_repository_snapshot(self) -> None:
        from control_plane import stable_pause
        ...
        with tempfile.TemporaryDirectory() as directory:
            repo = make_repository(Path(directory) / "repo")
            state, _ = install_lifecycle_fixture(repo)
            unmaterialized = GitStateMaterialization(
                ok=False,
                status="FAIL",
                scanned_files=1,
                dataless_files=1,
                areas=("objects",),
                truncated=False,
                error_code="E_MATERIALIZATION_DATALESS",
            )
            with patch(
                "control_plane.materialization.inspect_git_state_materialization",
                return_value=unmaterialized,
            ), patch.object(
                stable_pause,
                "_run_git",
                wraps=stable_pause._run_git,
            ) as observed_git:
                observed = stable_pause.observe_stable_pause(repo, state["task_id"])

            self.assertEqual(observed_git.call_count, 0)
