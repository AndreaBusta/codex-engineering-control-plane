...
No es “completamente óptimo”. Esta v1 quedó `review_ready` local tras `58/58`
tests y una revisión independiente sin hallazgos Critical o Important. No es un
raíl remoto hasta tener commit inicial, remote, CI y Ruleset; tampoco prueba
TestFlight hasta conectarse a Xcode Cloud/App Store Connect con autorización.
...
- Repositorio huérfano: todavía no existe baseline versionada.
- Sin remote ni protección efectiva de rama.
- Sin CI macOS/Linux.
...
- Commit inicial revisado y remote dedicado.
- Ruleset de GitHub con PR/checks obligatorios.
- CI para ejecutar `tests/run.sh`.
...
|---|---|---:|---:|---|---|
| R1 | escribir en base | media | alto | preflight write | Ruleset ausente |
| R2 | rama atrasada | media | alto | divergence gate + `--refresh` | remote no configurado |
| R3 | detached HEAD | media | alto | gate | commit inicial ausente |
| R4 | cambios mezclados | media | alto | skill + runbook | separación no automatizada |
| R5 | merge no remoto | alta histórica | alto | protocolo/recibo | GitHub no conectado |
...
| R8 | policy maliciosa | baja | alto | schema/nombres/unknown keys | review remota ausente |
