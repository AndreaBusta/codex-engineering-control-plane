...
| Git metadata is dataless, linked or becomes unobservable before Stable Pause | Git observation hydrates, skips or misreads state and certifies a changed substrate | Before any Stable Pause Git command, `.git` and `commondir` are parsed filesystem-first with bounded no-follow identity checks; descriptor-relative traversal bounds all entries, depth and cooperative time, while symlink/special/race is `UNKNOWN` and dataless regular state is `FAIL`. Every non-PASS becomes privacy-safe `UNKNOWN / E_STABLE_PAUSE_REPOSITORY`. |
...
| malicious Git config or filter redirects observation | Hostile helpers or filters execute or hide bytes | Fixed absolute Git and Python, closed environment, exact read-only forms, retained-directory `fchdir`, `GIT_OPTIONAL_LOCKS=0`, output bounded before buffering, process-group cleanup and direct blob verification. |
