...
| `AE-09` | RED: adoption sources and tests absent from the authoritative manifest | GREEN: closure requires one passing final-byte focal set, one authoritative gate, all post-gates and both independent rereviews on identical bytes | ROLLBACK: implementation rollback is path-exact; the bootstrap correction remains limited and version-bound | `CLOSES_ON_FINAL_EVIDENCE` |
...
Then run the authoritative local gate once:
...
- **Para continuar:** ejecutar los focales finales, una sola vez el gate integral, los post-gates y dos revisiones independientes sobre los mismos bytes; si todo pasa, AE-09 queda cerrado sin otra edición versionada.
